...
"""
        ...
        flag1 = True
        while flag1:
            choosed_option = input(menu_0)
            if choosed_option < "1" or choosed_option > "2":
                print("Incorrect data! Try again!\n")
            else:
                flag1 = False
                choosed_option = int(choosed_option)
    ...
    def sign_in(self):
        ...
        # Check the passwords
        flag_1 = False
        while flag_1 is False:  # Untill both "password" and "repeated_password" isn't the same.
            ...
            hashing_object.update(byte_password)
            hashed_password = hashing_object.hexdigest()
            ...
            # Get number of sign users. Opening file is protected.
            ...
            # Save data into users db file.  Opening file is protected.
            try:
                ...
                with open("users\\users_database.txt", "a") as file_2:  # Open "users_database.txt" file for append.
                    ...
                    file_2.write(how_many_user + "=" + username + "=" + native_language + "=" + hashed_password + "\n")  # Append all user data into "users_database.txt" file.
    ...
    def check_in_user_db(self, data, what_data, all_data = False):
        ...
        # Get data from "users_database" file. Opening file is protected.
        try:
            with open("users\\users_database.txt", "r") as file_1:  # Open "users_database.txt" for read.

                # Check all data by getted value and return True if exist, False if not, also can added return all data by getted data.
                for line in file_1:
                    users_data.append(line.rsplit("="))  # Separate line from file
        ...
        # Check datas from "users_database.txt" file, by specified value.
        for data_line in users_data:
            if what_data == 0:  # Check only first datas (indexes).
                if data == int(data_line[what_data]):  # If proper index already exist in file "users_database.txt"
                    all.append(data_line)  # Append all data to list "all"
                    flag = True  # Finish loop.
            elif what_data == 1:  # Check only second datas (username).
                if data == data_line[what_data]:  # If proper login already exist in file "users_database.txt"
                    all.append(data_line)  # Append all data to list "all"
                    flag = True  # Finish loop.
            elif what_data == 2:  # Check only third datas (native_language).
                if data == data_line[what_data]:  # If proper native language already exist in file "users_database.txt"
                    all.append(data_line)  # Append all data to list "all"
                    flag = True  # Finish loop.
            elif what_data == 3:  # Check only fourth datas (password).
                if data == data_line[what_data]:  # If proper password already exist in file "users_database.txt"
                    all.append(data_line)  # Append all data to list "all"
                    flag = True  # Finish loop.
    ...
    def log_in(self):
        ...
        while flag == False:
            ...
            if d_1 is False:  # If there is no user's "username" in "users_database.txt" file.
                ...
                if hashed_password != setted_password:  # If hashed_password are not the same as setted password.
                    decision = input("Access denied! Do you want to exit program? (y/n): ")  # Get info from user if he want to try again or not.
                    if decision == "y": exit(0)  # If user don't want to log in again, exit program.
