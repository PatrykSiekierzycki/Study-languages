...
Give a number: """
        ...
        flag2 = True
        while flag2:
            new_words_or_repeats = int(input(question_about_repeats))
    ...
    def learn_new_words(self):
        ...
        os.chdir("..")
        os.chdir("..")
        file_name = self.list_of_dictionary_files[self.option_number - 1] # Name of user's choosed language file.
        self.read_dict_file(file_name)  # Call method "read_dict_file".
    ...
    def repeat_words(self):
        file_name = self.list_of_users_dicts_file[self.option_number - 1] # Name of user's choosed language file
        self.read_user_file(file_name)  # Call method "read_user_file".
    ...
    Return: None. 
    """
    def read_dict_file(self, file_name): # # Name of user's choosed language file.

        checkIndexInFile_object = CheckIndexInFile(self.path_to_user_file, file_name)  # Create an CheckIndexInFile's object. Provide path to users file and a name of file dictionary file.
    ...
    Return: None.
    """
    def read_user_file(self, file_name):

        # Protection before unexisted user file.
        if os.path.exists(self.list_of_dictionary_files[self.option_number]):
            print("File dosen't exist! You should first choose an option to learn new words.")

        # Preper list of dict from users file
        get_list_of_dicts_object = Get_list_of_dicts(file_name)
        prepered_list = get_list_of_dicts_object.preper_list() # All word from user's file
        ...
        get_word_to_repeat_object = Get_word_to_repeat(file_name)  # Creat a "Get_word_to_repeat" object and provide to it a "file_name".
...
class Get_word_to_repeat:
    ...
    def update_users_file(self, updated_data):
        ...
        unupdated_list = get_list_of_dicts_object.preper_list()  # Get a list with dicts of all words from user's file.
...
class Get_list_of_dicts:
    ...
    def preper_list(self):
        ...
        return prepered_list_of_dicts  # After loop return finished list of dicts.
